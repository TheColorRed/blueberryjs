namespace Canvas {
    export class Input {
                ...
                if (input.name == value) {
                    if (input.keys) {
                        ...
                        for (let k = 0; k < len2; k++) {
                            if (input.keys.positive && Input.keyPressed(input.keys.positive[k])) {
                                return 1;
                            }
                        }
                        let len3 = (input.keys.negative || []).length;
                        for (let k = 0; k < len3; k++) {
                            if (input.keys.negative && Input.keyPressed(input.keys.negative[k])) {
                                return -1;
                            }
                        }
                    }
        ...
        public static addButton(name: string, input: { key?: number, mouse?: number }, isPositive: boolean = true) {
            ...
            if (obj) {
                if (isPositive) {
                    if (input.key && obj.keys && obj.keys.positive) {
    ...
    export enum MouseState { None, Pressed, Released };
